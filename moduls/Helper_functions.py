...
def plot_images_clas_toy(num_rows: int = 3,
                     num_col: int = 3,
                     fig_size: float = 2,
                     images=None,
                     labels=None,
    ...
    if num_rows * num_col > len(images):
        print(f"You have selected {num_col * num_rows} images to show but you have only {len(images)}.")
        return None
...
def plot_predictions_linear(train_data,
                            train_labels,
                            test_data,
                            test_labels,
                            predictions,
    ...
    """
    if convert_cpu and (predictions == None):
...
def plot_loss_curve(dict_lr):
    plt.figure(figsize=(10, 8))

    for key, value in dict_lr.items():
        plt.scatter(key, value[0], c="b", s=4, label="Training loss")
        plt.scatter(key, value[1], c="g", s=4, label="Validation loss")
